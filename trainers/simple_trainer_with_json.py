# =============================================================================
# FILE 2: trainers/simple_trainer_with_json.py
# =============================================================================
"""
Enhanced SimpleTrainer with built-in JSON logging and validation support.
Clean implementation without complex method overriding.
"""
...
class SimpleTrainerWithJSON(SimpleTrainer):
    """
    Enhanced SimpleTrainer with JSON logging and validation.
    """
    ...
    def __init__(self, 
                 ...
                 validate_every_n_batches: int = None):  # New parameter
        ...
        self.validate_every_n_batches = validate_every_n_batches  # Validate every N batches
        
        # Global step counter for JSON logging
        self.global_step = 0
    ...
    def _train_epoch(self, epoch: int) -> float:
        ...
        for batch_idx, batch in enumerate(self.dataloader):
            ...
            # Batch logging (every log_interval batches)
            if batch_idx % self.log_interval == 0:
                self.log_batch(batch_idx, batch_loss, epoch, metrics={'perplexity': batch_perplexity})
            
            # JSON batch logging (every 50 batches)
            if self.json_logger and batch_idx % 50 == 0:
            ...
            if (self.validate_every_n_batches and 
                ...
                batch_idx > 0 and 
                batch_idx % self.validate_every_n_batches == 0):
                
                logger.info(f"Running mini-validation at batch {batch_idx}...")
                val_metrics = run_validation(self.model, self.val_dataloader, self.device, max_batches=10)  # Quick validation
                val_loss = val_metrics['loss']
                val_perplexity = val_metrics['perplexity']
    ...
    def log_batch(self,
        ...
        Args:
            ...
            metrics (dict, optional): Additional metrics to log (should include perplexity).
        """
